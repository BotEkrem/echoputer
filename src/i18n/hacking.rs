...
pub const SNAP_SAVED: Msg = ["snapshot", "goruntu"];
pub const CAM_CONTROL_HINT: Msg = ["C: control camera", "C: kamerayi yonet"];
pub const CAM_CONTROL: Msg = ["Camera Control", "Kamera Kontrol"];
pub const CAM_CTL_HINT: Msg = ["arrows PTZ   SPACE snap   ESC exit", "oklar PTZ   SPACE goruntu   ESC cik"];
pub const ESC_EXIT: Msg = ["ESC exit", "ESC cik"];
pub const PTZ_NA: Msg = ["n/a", "yok"];
pub const PTZ_OK: Msg = ["ok", "tamam"];
pub const PTZ_FAIL: Msg = ["fail", "hata"];
pub const PTZ_READY: Msg = ["ready", "hazir"];
...
pub const DEAUTH_NA: Msg = ["deauth blocked by HW", "deauth HW'de bloklu"];
pub const USE_HANDSHAKE: Msg = ["use Handshake / Evil Twin", "Handshake / Evil Twin kullan"];
